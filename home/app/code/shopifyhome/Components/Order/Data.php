...
class Data extends Common
{
    ...
    public function prepare($userId = null): void{
        ...
        $mongo->setSource("order_container_" . $userId);

        $this->_collection = $mongo->getPhpCollection();
        $this->_userId = $userId;
    }

    public function handleOrderData($data, $userId = null){
        $this->prepare($userId);
    ...
    public function updateOrder($data): void{
        ...
        $out = $this->_collection->findOne(['details.source_order_id' => $data['source_order_id']], ['typeMap' => ['root' => 'array', 'document' => 'array']]);

        $helper = $this->di->getObjectManager()->get(Helper::class);
        $data = $helper->formatCoreDataForOrderData($data);


        if(empty($out)){
            $res = $this->_collection->updateOne(
                [ 'details.source_order_id' => $data['id']],
                [
                    '$set' => [
            ...
            $res = $this->_collection->updateOne(
                [ 'details.source_order_id' => (string)$data['source_order_id'] ],
                [ '$set' => [
                    'details.$' => $data,
                    'details.is_imported' => 1
                ]
