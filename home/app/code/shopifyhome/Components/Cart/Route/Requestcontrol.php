...
namespace App\Shopifyhome\Components\Cart\Route;

use App\Shopifyhome\Components\Cart\Helper;
use App\Shopifyhome\Components\Cart\Import;
use App\Core\Components\Base;
use App\Connector\Models\QueuedTasks;
class Requestcontrol extends Base
{
    public const SHOPIFY_QUEUE_CART_IMPORT_MSG = 'SHOPIFY_ABANDONED_CHECKOUT_IMPORT';

    public function fetchShopifyCart($data){
        if (!isset($data['data']['cursor'])) {
            $remoteResponse = $this->di->getObjectManager()->get("\App\Connector\Components\ApiClient")->init('shopify', 'true')->call('/abandoned/checkouts/count',[],['shop_id'=> $data['data']['remote_shop_id']], 'GET');
            ...
            if ($remoteResponse['success'] && $remoteResponse['data'] == 0) {
                ...
                if ($progress && $progress == 100) {
                    $this->di->getObjectManager()->get('\App\Connector\Components\Helper')->addNotification($data['data']['user_id'], 'No Abandoned Checkout Found on your Shopify Store', 'critical');
                }
            ...
            if ($remoteResponse['success']) {
                ...
                $this->di->getObjectManager()->get('\App\Connector\Components\Helper')->updateFeedProgress($data['data']['feed_id'], 0, Requestcontrol::SHOPIFY_QUEUE_CART_IMPORT_MSG.' : '.$remoteResponse['data'].  ' cart(s) found. Please wait while carts are being imported.');
                //resetData
                ...
                $progress = $this->di->getObjectManager()->get('\App\Connector\Components\Helper')->updateFeedProgress($data['data']['feed_id'], 100);
                if ($progress && $progress == 100) {
                    $this->di->getObjectManager()->get('\App\Connector\Components\Helper')->addNotification($data['data']['user_id'], 'Failed to fetch Cart(s) from Shopify. Please try again later.', 'critical');
                }
        ...
        if (isset($data['data']['delete_nexisting'])) {
            $this->di->getObjectManager()->get(Helper::class)->deleteNotExistingCart();
            $this->di->getObjectManager()->get('\App\Connector\Components\Helper')->updateFeedProgress($data['data']['feed_id'], 5, 'Finalizing .. ');
            $this->di->getObjectManager()->get('\App\Connector\Components\Helper')->addNotification($data['user_id'], $data['data']['total']. ' Shopify Checkout(s) has been successfully imported.', 'success');
            return true;
        }
        else {
            ...
            $remoteResponse = $this->di->getObjectManager()->get("\App\Connector\Components\ApiClient")->init('shopify', 'true')->call('/abandoned/checkouts',[],$filter, 'GET');

            $handleCartData = $this->di->getObjectManager()->get(Import::class);
            if($remoteResponse['success'] && isset($remoteResponse['data'])) {
                $response = $handleCartData->getShopifyCarts($remoteResponse, $userId);
            } elseif (count($remoteResponse) > 9) {
                $response = $handleCartData->getShopifyCarts(['data' => $remoteResponse], $userId);
            } else {
                $response['success'] = false;
            }
            if ($response['success']) {
                $data['data']['cursor'] = $data['data']['cursor'] +1 ;
                $msg = Requestcontrol::SHOPIFY_QUEUE_CART_IMPORT_MSG.' : '.($data['data']['cursor'] * 250).' of '.$data['data']['total'].' Shopify Checkout(s) has been successfully imported.';
                ...
                if(($data['data']['cursor'] * 250) >= $data['data']['total']){
                    ...
                    $this->di->getObjectManager()->get('\App\Connector\Components\Helper')->updateFeedProgress($data['data']['feed_id'], 0, Requestcontrol::SHOPIFY_QUEUE_CART_IMPORT_MSG.' :  Finalizing .. ');
                    $this->pushToQueue($data);
