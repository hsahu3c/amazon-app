...
use App\Shopifyhome\Components\Cart\Route\Requestcontrol;
...
class Helper extends Common
{
    ...
    public const SHOPIFY_QUEUE_CART_IMPORT_MSG = 'SHOPIFY_CART_IMPORT';

    public function formatCoreDataForDetail($oData)
    {
        $oData['source_cart_id'] = (string)$oData['id'];
        $oData['is_imported'] = 1;
        unset($oData['id']);
        return $oData;
    }
    ...
    {
            $lIData['source_cart_id'] = (string)$lIData['cart_id'];
    ...
    public function shapeSqsData($userId, $remoteShopId, QueuedTasks $queuedTask)
    {
        $handlerData = [
            ...
            'method' => 'fetchShopifyCart',
            'queue_name' => 'shopify_cart_import',
            'own_weight' => 100,
            'user_id' => $userId,
            'data' => [
                'user_id' => $userId,
    ...
    public function resetImportFlag($userId = '')
    {
        ...
        $collection = $mongo->setSource("cart_container_".$userId)->getPhpCollection();
    ...
    public function deleteNotExistingCart($userId = '', $append = '')
    {
        $userId = !empty($userId) ? $userId : $this->di->getUser()->id;
        $mongo = $this->di->getObjectManager()->create('\App\Core\Models\BaseMongo');
        $collection = $mongo->setSource("cart_container_".$userId)->getPhpCollection();
        $query = ['details.is_imported' => 0]; $options = ['projection' => ['details.source_cart_id' => 1 ]  ];
        $cursor = $collection->find($query, $options);
        $sourceCartId = [];
        $response = [];
        foreach ($cursor as $document) {
            if(isset($document['details']['source_cart_id'])) {
                $sourceCartId[] = $document['details']['source_cart_id'];
            }
        }
        foreach ($sourceCartId as $id){
            $cartContainer = $this->di->getObjectManager()->get('\App\Connector\Models\CartContainer');
            $response = $cartContainer->deleteCart(['details.source_cart_id' => $id]);
        }

        return true;
    }

    public function formatCoreDataForCartData($vData)
    {
        $vData['source_cart_id'] ??= (string)$vData['id'];
        $vData['is_imported'] = 1;
        $date = new DateTime("now", new DateTimeZone('Asia/Kolkata') );
