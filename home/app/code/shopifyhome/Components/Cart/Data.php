...
class Data extends Common
{
    ...
    public function prepare($userId = false): void{
        ...
        $mongo->setSource("cart_container_" . $userId);

        $this->_collection = $mongo->getPhpCollection();
        $this->_userId = $userId;
    }

    public function handleCartData($data, $userId = null){
        $this->prepare($userId);
    ...
    public function updateCart($data): void{
        ...
        $out = $this->_collection->findOne(['details.source_cart_id' => $data['source_cart_id']], ['typeMap' => ['root' => 'array', 'document' => 'array']]);

        $helper = $this->di->getObjectManager()->get(Helper::class);
        $data = $helper->formatCoreDataForCartData($data);


        if(empty($out)){
            $res = $this->_collection->updateOne(
                [ 'details.source_cart_id' => $data['id']],
                [
                    '$set' => [
            ...
            $res = $this->_collection->updateOne(
                [ 'details.source_cart_id' => (string)$data['source_cart_id'] ],
                [ '$set' => [
                    'details.$' => $data,
                    'details.is_imported' => 1
                ]
