...
class AmazonPriceSyncCommand extends Command
{

    protected static $defaultName = "price-sync";
    protected $di;
    protected static $defaultDescription = "initiate Price sync";
    ...
    {
        $this->setHelp('initiate Price sync');
    }
    ...
    {
        print_r("Executing command to initiate Price sync \n");
        $this->requestPrice();
        return 0;
    }


    public function requestPrice(): void
    {
        $cronTask = 'Price sync';
        $limit = 100;
        $skip = 0;
        $continue = true;
        $price = $this->di->getObjectManager()->get('\App\Amazon\Components\Price\Price');
        ...
        $pricesyncUsers = $this->di->getConfig()->get('PremiumPriceUser') ?? [];
        $pricesyncUsers = json_decode(json_encode($pricesyncUsers),true);
        do {
            $options = [
                'limit' => $limit,
                'skip' => $skip,
