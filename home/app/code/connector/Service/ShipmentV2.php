...
class ShipmentV2 extends AbstractShipment implements ShipInterface
{
    const SOURCE = 'source';
    const TARGET = 'target';
            ...
            if (!empty($targetData)) {
                ...
                if ($this->objectType == self::TARGET_ORDER) {
                    ...
                    if (isset($targetData['shipment_error'])) {
                        $this->orderCollection->updateOne(['_id' => $targetData['_id']], ['$set' => ["shipping_status" => self::STATUS, "updated_at_iso" => new UTCDateTime((int)(microtime(true) * 1000))]]);
                    }
                    $prepareData = [
                        'object_type' => self::TARGET_SHIPMENT,
                        'marketplace' => $data['target_marketplace'],
            ...
            if (!empty($sourceData)) {
                ...
                if ($this->objectType == self::SOURCE_ORDER) {
                    ...
                    if (isset($sourceData['shipment_error'])) {
                        $this->orderCollection->updateOne(['_id' => $sourceData['_id']], ['$set' => ["shipping_status" => self::STATUS, 'updated_at_iso' => new UTCDateTime((int)(microtime(true) * 1000))]]);
                    }
                    $prepareData = [
                        'object_type' => self::SOURCE_SHIPMENT,
                        'shop_id' => $data['source_shop_id'],
    ...
    {
        $saveShipment = $this->orderCollection->insertOne($shipmentData);
    ...
    private function updateShipmentDoc($filter, $shipmentData)
    {
        ...
        $this->orderCollection->updateOne($filter, ['$set' => $shipmentData]);
    }
